...
async def create_card(
    ...
    list_obj = await card_service.get_by_id(db, card_in.list_id)
    if not list_obj:
        raise HTTPException(
...
async def move_card(
    ...
    dest_list = await card_service.get_by_id(db, card_move.list_id)
    if not dest_list:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def reorder_cards(
    ...
    # Get list to check board access
    list_obj = await card_service.get_by_id(db, list_id)
    if not list_obj:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="List not found"
